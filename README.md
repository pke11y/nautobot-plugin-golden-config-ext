...
```shell
pip install golden-config-extension
```

> The plugin is compatible with Nautobot 1.3.5 and higher

To ensure PK Playground is automatically re-installed during future upgrades, create a file named `local_requirements.txt` (if not already existing) in the Nautobot root directory (alongside `requirements.txt`) and list the `golden-config-extension` package:

```no-highlight
# echo golden-config-extension >> local_requirements.txt
```
...
PLUGINS = ["golden_config_extension"]
...
- `project_name`: the default docker compose project name (default: golden_config_extension)
...
Each command can be executed with `invoke <command>`. Environment variables `INVOKE_golden_config_extension_PYTHON_VER` and `INVOKE_golden_config_extension_NAUTOBOT_VER` may be specified to override the default versions. Each command also has its own help `invoke <command> --help`
